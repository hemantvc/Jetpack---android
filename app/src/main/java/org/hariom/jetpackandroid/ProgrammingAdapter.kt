...
class ProgrammingAdapter :
    ListAdapter<ProgrammingItem, ProgrammingAdapter.ProgrammingViewHolder>(DiffUtilCallback()) {
    ...
    class ProgrammingViewHolder(view : View) : RecyclerView.ViewHolder(view){
        ...
        val intital = view.findViewById<TextView>(R.id.tvInital)

        fun bind(item : ProgrammingItem){
            name.text =  item.name
            intital.text = item.intial
        }
    }
